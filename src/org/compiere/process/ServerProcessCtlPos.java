...
public class ServerProcessCtlPos extends ServerProcessCtl {
		...
		public ServerProcessCtlPos(ASyncProcessPOS parent, ProcessInfo pi, Trx trx2) {
			super(pi, trx2);
		}
		 ...
		 */
		public static ServerProcessCtl process (ASyncProcessPOS parent, ProcessInfo pi, Trx trx)
		{
			...
			//	execute
			ServerProcessCtlPos worker = new ServerProcessCtlPos(parent, (ProcessInfoPOS) pi, trx);
			if (parent != null)
			{
				//asynchrous
				worker.start();
		...
		public void run ()
		{
			...
			if (IsReport)
			{
				processInfo.setReportingProcess(true);
				//	Start Report	-----------------------------------------------
				boolean ok = ServerReportCtlPos.start(m_parent, processInfo);
		...
		protected boolean startWorkflow (int AD_Workflow_ID)
		{
			...
			if (isServerProcess)
			{
				...
				try
				{
					if (server != null)
					{	//	See ServerBean
						processInfo = (ProcessInfoPOS) server.workflow (Env.getRemoteCallCtx(Env.getCtx()), processInfo, AD_Workflow_ID);
		...
		protected boolean startProcess ()
		{
			...
			if (isServerProcess && !clientOnly)
			{
				...
				try
				{
					...
					{	
						//	See ServerBean
						processInfo = (ProcessInfoPOS) server.process (Env.getRemoteCallCtx(Env.getCtx()), processInfo);
						log.finest("server => " + processInfo);
						started = true;		
					}
				}
		...
		protected boolean startDBProcess (String ProcedureName)
		{
			...
			if (isServerProcess)
			{
				...
				try
				{
					if (server != null)
					{	//	See ServerBean
						processInfo = (ProcessInfoPOS) server.dbProcess(null, processInfo, ProcedureName);
						log.finest("server => " + processInfo);
						started = true;		
					}
				}

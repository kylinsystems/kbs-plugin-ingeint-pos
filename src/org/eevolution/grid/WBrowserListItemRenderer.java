...
public class WBrowserListItemRenderer implements ListitemRenderer, EventListener, ListitemRendererExt , ValueChangeListener
{
									  ...
									  int rowIndex, int columnIndex)
	{
		...
		if (isColumnVisible && gridField != null) {
					...
					&& !browseField.isKey())
			{
				if (isCellEditable)
				{
					WEditorPOS editor = WebEditorFactoryPOS.getEditor(gridField, true);
	...
	public void valueChange(ValueChangeEvent evt) {
		if (evt.getSource() instanceof WEditorPOS) {
			GridField changedField = ((WEditorPOS) evt.getSource()).getGridField();
		...
		{
			WEditorPOS wEditor = (WEditorPOS)evt.getSource();

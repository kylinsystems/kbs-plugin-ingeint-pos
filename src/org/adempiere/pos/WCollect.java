...
//		fPayAmt.setText(getPrePayAmt().toString());
		...
		// Button Plus
		bPlus = createButtonAction("Editor", KeyStroke.getKeyStroke(KeyEvent.VK_F3, Event.F3));
		row = rows.newRow();
...
//			}
	...
	protected Button createButtonAction (String action, KeyStroke accelerator)	{
		...
		//button.setZclass("z-bandbox-icon z-icon-upload");
	...
	public void printTicketWeb() {	
		...
		try {
			...
			if (posPanel.isToPrint() && posPanel.hasOrder()) {
				ReportCtlPOS.startDocumentPrint(0, posPanel.getC_Order_ID(), false);
				...
				AMedia media = new AMedia(m_reportEngine.getPrintFormat().getName() + ".txt", null, "text/html", data);
				//TODO Verificar impresión directa  application/octet-stream

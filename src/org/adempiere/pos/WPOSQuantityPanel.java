...
//			buttonScales.addActionListener(posPanel.getScalesListener());
		...
		fieldQuantity = new WNumberEditor();
		
		row.appendChild(fieldQuantity.getComponent());
		fieldQuantity.getComponent().addEventListener(Events.ON_OK, this);
		fieldQuantity.getComponent().addEventListener(Events.ON_CHANGE, this);
		fieldQuantity.getComponent().setStyle("display: inline;width:65px;height:30px;Font-size:medium;");

		Label priceLabel = new Label(Msg.translate(Env.getCtx(), "PriceActual"));
		row.appendChild(priceLabel);
		
		fieldPrice = new WNumberEditor();
		DecimalFormat format = DisplayType.getNumberFormat(DisplayType.Amount, AEnv.getLanguage(Env.getCtx()));
		
		fieldPrice.getComponent().setTooltiptext(Msg.translate(Env.getCtx(), "PriceActual"));
		row.appendChild(fieldPrice.getComponent());
		if (!posPanel.isModifyPrice())
			fieldPrice.getComponent().setEnabled(false);
		else {
			fieldPrice.getComponent().addEventListener(Events.ON_OK, this);
			fieldPrice.getComponent().addEventListener(Events.ON_CHANGE, this);
		}
		fieldPrice.getComponent().setStyle("display: inline;width:70px;height:30px;Font-size:medium;");
		
		Label priceDiscount = new Label(Msg.translate(Env.getCtx(), "Discount"));
		row.appendChild(priceDiscount);

		fieldDiscountPercentage = new WNumberEditor();
		row.appendChild(fieldDiscountPercentage.getComponent());
		fieldDiscountPercentage.getComponent().setTooltiptext(Msg.translate(Env.getCtx(), "Discount"));
		if (!posPanel.isModifyPrice())
			fieldDiscountPercentage.getComponent().setEnabled(false);
		else{
			fieldDiscountPercentage.getComponent().addEventListener(Events.ON_OK, this);
			fieldDiscountPercentage.getComponent().addEventListener(Events.ON_CHANGE, this);
		}
		fieldDiscountPercentage.getComponent().setStyle("display: inline;width:70px;height:30px;Font-size:medium;");
		
		Keylistener keyListener = new Keylistener();
		fieldPrice.getComponent().appendChild(keyListener);
	...
	public void onEvent(Event e) throws Exception {
		try {
			if (Events.ON_CTRL_KEY.equals(e.getName())) {
				...
				if (keyEvent.getKeyCode() == 114 ) {
					...
					fieldQuantity.getComponent().setValue(0.0);
					fieldPrice.getComponent().setValue(0.0);
					fieldDiscountPercentage.getComponent().setValue(0.0);
				}
				//ctrl+1 == 49
				if (keyEvent.getKeyCode() == 49 ) {
					fieldQuantity.getComponent().setValue(fieldQuantity.getComponent().getValue().add(CurrentQuantity));
				}
				//ctrl+0 == 48
				if (keyEvent.getKeyCode() == 48 ) {
					fieldQuantity.getComponent().setValue(fieldQuantity.getComponent().getValue().subtract(CurrentQuantity));
				}
			}
			...
			if (e.getTarget().equals(buttonMinus)){
				BigDecimal quantity = fieldQuantity.getComponent().getValue().subtract(CurrentQuantity);
				...
				posPanel.setQty(fieldQuantity.getComponent().getValue().add(CurrentQuantity));
			}
			...
			if(Events.ON_OK.equals(e.getName()) || Events.ON_CHANGE.equals(e.getName())) {
			  ...
			  value = fieldQuantity.getComponent().getValue();
        if(value == null)
          return;
        if(e.getTarget().equals(fieldQuantity.getComponent().getDecimalbox())) {
        ...
        if (e.getTarget().equals(fieldPrice.getComponent().getDecimalbox())) {
          value = fieldPrice.getComponent().getValue();
          if(value == null)
            return;
          if(posPanel.isUserPinValid()) {
        ...
        else if ( e.getTarget().equals(fieldDiscountPercentage.getComponent().getDecimalbox())) {
          if(posPanel.isUserPinValid()) {
            value = fieldDiscountPercentage.getComponent().getValue();
            if(value == null)
              return;
	...
	public void changeStatus(boolean status) {
		fieldQuantity.getComponent().setEnabled(true);
		fieldPrice.getComponent().setEnabled(true);
		fieldDiscountPercentage.getComponent().setEnabled(true);
		...
		if(posPanel.hasLines()){
			...
			if(posPanel.getOrder().getDocStatus().compareToIgnoreCase(MOrder.STATUS_Drafted)==0 || 
			   posPanel.getOrder().getDocStatus().compareToIgnoreCase(MOrder.STATUS_InProgress)==0 ){
				...
				fieldPrice.getComponent().setEnabled(false);
				fieldQuantity.getComponent().setEnabled(false);
				fieldDiscountPercentage.getComponent().setEnabled(false);
			}
		} else {
	...
	public void changeViewPanel() {
		if(posPanel.getQty().compareTo(Env.ZERO) == 0)
			...
			changeStatus(true);
		fieldQuantity.getComponent().setValue(posPanel.getQty());
		fieldPrice.getComponent().setValue(posPanel.getPrice());
		fieldDiscountPercentage.getComponent().setValue(posPanel.getDiscountPercentage());
	}

	public void resetPanel() {
		fieldQuantity.getComponent().setValue(0);
		fieldPrice.getComponent().setValue(0);
		fieldDiscountPercentage.getComponent().setValue(0);
		buttonDown.setEnabled(false);
		buttonUp.setEnabled(false);
		buttonDelete.setEnabled(false);
		buttonPlus.setEnabled(false);
	...
	public void setQuantity(BigDecimal value) {
		fieldQuantity.getComponent().setValue(value);
		fieldQuantity.getComponent().focus();
	}

...
public class WPOSBPartner extends Window implements EventListener, ValueChangeListener
{
	...
	/**	Default Font		*/
	private final String 	FONT_SIZE = "Font-size:small;";
	/**	Default Width		*/
	...
	private void initBPartner()
	{
		...
		// TaxIdType
		v_TaxIDTypePanel = GridFactory.newGridLayout();
		v_TaxIDTypePanel.setWidth("100%");
		v_TaxIDTypePanel.setHeight("195px");

		Rows rows = v_TaxIDTypePanel.newRows();
		Row row = rows.newRow();

		row.setStyle("350px");

		int AD_Column_ID = DB.getSQLValue(pos.get_TrxName(), "SELECT AD_Column_ID "
				+ "FROM AD_Column "
				+ "WHERE columnname='LCO_TaxIdType_ID' "
				+ "AND AD_Table_ID = 291");	//LCO_TaxIdType		

		if (AD_Column_ID>1) {		

			isLCO = true;
			MLookup taxtypelookup = MLookupFactory.get(Env.getCtx(), 0, 0, AD_Column_ID, DisplayType.TableDir);
			ArrayList<Object> taxidtypes = taxtypelookup.getData(true, false, true, true, false);

			row = rows.newRow();
			fTaxIDType=ListboxFactory.newDropdownListbox();
			row.appendChild(fTaxIDType);
			fTaxIDType.setStyle(HEIGHT+WIDTH+FONT_SIZE);
			fTaxIDType.setValue(Msg.translate(getContext(), "CreditCardType"));
			fTaxIDType.addActionListener(this);		

			for (Object obj : taxidtypes) {
				if ( obj instanceof KeyNamePair )	{
					KeyNamePair key = (KeyNamePair) obj;
					fTaxIDType.appendItem(key.getName(), key.getID());			
				}
			}
	...
	private boolean actionSave()
	{
		...
		if (partner == null)
		{
			...
			if(isLCO)
				partner.set_ValueOfColumn("LCO_TaxIdType_ID", fTaxIDType.getValue());
			partner.setTaxID(fTaxID.getValue());
		}
		...
		if (m_user != null)
		{
			...
			if (m_user.save()) {
				...
				pos.configureBPartner(partner.get_ID());
				pos.refreshHeader();
			}else
	...
	public String showKeyboard(Event e){
		...
		if(keyboard != null){
			if(e.getName().equals(Events.ON_FOCUS)){
				...
				keyboard.setAttribute(Window.MODE_KEY, Mode.OVERLAPPED);

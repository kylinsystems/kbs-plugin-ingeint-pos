...
public class WCollectDetail extends CollectDetail implements EventListener, POSPanelInterface {
	...
	public void loadStandardPanel(){
		...
		bMinus = v_Parent.createButtonAction("Delete", KeyStroke.getKeyStroke(KeyEvent.VK_F3, Event.F3));
	...
	private void loadCreditMemoPanel() {
		...
		ArrayList<Object> types = lookup.getData(false, false, true, true, true);
	...
	public void onEvent(org.zkoss.zk.ui.event.Event e) throws Exception {
		if(e.getTarget().equals(bMinus)){
			...
			changeViewPanel();
			bAmountField.setValue(getInitPayAmt());
			setPayAmt((BigDecimal) bAmountField.getValue());
...
//			setDateTrx(dateTrx);
			...
			v_Parent.refreshPanel();
			bAmountField.setValue(getOpenAmtCreditMemo());
			
			setPayAmt((BigDecimal) bAmountField.getValue());
			v_Parent.refreshPanel();
		} else if(e.getName().equals(Events.ON_FOCUS)){
			if(e.getTarget().equals(fCheckRouteNo.getComponent(WPOSTextField.SECONDARY)) && !isKeyboard) {
				isKeyboard = true;
			...
			BigDecimal payAmt = (BigDecimal)bAmountField.getValue();
			
			if(Events.ON_CHANGE.equals(e.getName()) || e.getName().equals(Events.ON_CHANGING)) {
				if(((InputEvent)e).getValue().length() > 0) {
					String number2	 = ((InputEvent)e).getValue().toString();
					double val = Double.parseDouble(number2.replaceAll(",", "."));
					payAmt = new BigDecimal(val);
			
				}
				else 
					payAmt = Env.ZERO;

				if(p_TenderType.equals("M")
	...
	//  Change View
		changeViewPanel();
		bAmountField.setValue(getPayAmt());
	...
	public void changeViewPanel() {
		...
		if(p_TenderType == null)
			return;
		//	Change Title
		String m_DisplayTenderType = fTenderType.getName();

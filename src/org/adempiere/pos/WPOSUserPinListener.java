...
public class WPOSUserPinListener implements EventListener {
    ...
    protected void doPerformAction(Event e)
    {
        ...
        WNumberEditor number = null;
        Button button = null;

        if (objectSource instanceof WNumberEditor)
            number = (WNumberEditor) objectSource;
        ...
        if (
               ...
               (number != null && (Msg.translate(Env.getCtx(), "Discount").equals(number.getComponent().getTooltiptext()) 
            		   || Msg.translate(Env.getCtx(), "PriceActual").equals(number.getComponent().getTooltiptext()))
           ||  //Button validation

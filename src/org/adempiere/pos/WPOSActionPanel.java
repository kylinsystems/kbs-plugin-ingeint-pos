...
implements POSKeyListener, POSPanelInterface, POSQueryListener ,  POSLookupProductInterface, ValueChangeListener{
	...
	public void init() {
		...
		isKeyboard = false;
		//LayoutButton.setStyle("border: none; width:100%; height:95%;");
		...
		// NEW
		buttonNew = createButtonAction(ACTION_NEW, "F2", null);
		buttonNew.addActionListener(this);
		row.appendChild(buttonNew);

		// PRINT
		buttonPrint = createButtonAction(ACTION_PRINT, "F12", null);
		buttonPrint.addActionListener(this);
		row.appendChild(buttonPrint);

		// DocType 
		buttonDocType = createButtonAction(ACTION_DOCTYPE, "F10",null);
		...
		// PRODUCT
		buttonProduct = createButtonAction(ACTION_PRODUCT, "Alt+I", null);
		...
		// BPartner Search
		buttonBPartner = createButtonAction(ACTION_BPARTNER, "Alt+B", null);
		...
		buttonProcess = createButtonAction(ACTION_PROCESS, "Alt+P", null);
		...
		// Cancel
		buttonCancel = createButtonAction (ACTION_CANCEL, "F3", null);
		...
		// LOGOUT
		buttonLogout = createButtonAction (ACTION_LOGOUT, "Alt+L", null);
		...
		lookupPR = MLookupFactory.get (Env.getCtx(), posPanel.getWindowNo(), 0, AD_Column_ID, DisplayType.Search);
		fldProduct = new WSearchEditor("M_Product_ID", true, false, true, lookupPR);
		...
		// InfoOrder		
		int AD_OColumn_ID = 2161;
		lookupOV = MLookupFactory.get (Env.getCtx(), posPanel.getWindowNo(), 0, AD_OColumn_ID, DisplayType.Search);
		fldOrder = new WSearchEditor("C_Order_ID", true, false, true, lookupOV);
		fldOrder.addValueChangeListener(this);
		...
		row = rows.newRow();
		row.setSpans("12");
		fldProduct.addValueChangeListener(this);
		row.appendChild(fldProduct.getComponent());
		...
		if (posPanel.isEnableProductLookup() && !posPanel.isVirtualKeyboard()) {
			...
			lookupProduct.setVisible(false);
	...
	private void openDocType() { 
		...
		qt.setAttribute(Window.MODE_KEY, Mode.OVERLAPPED);
		qt.setVisible(true);
		AEnv.showWindow(qt);
	}
	...
	private void openHistory() { 
		...
		fldOrder.getComponent();

		InfoPanel ip = InfoManager.create(lookupOV, null, "C_Order", "C_Order_ID", "", false, "C_Pos_ID Notnull AND DocStatus IN ('DR', 'IP')");
		if (ip!=null)
			fldOrder.showInfoPanel(ip);
	...
	public void valueChange (ValueChangeEvent e) {
		...
		if (e.getPropertyName().equals("M_Product_ID")) {
			if (e.getNewValue()!=null) {
				...
				posPanel.refreshProductInfo(M_Product_ID);
				posPanel.addOrUpdateLine(M_Product_ID, Env.ONE);
				fldProduct.setValue(null);
			}
		}
		if (e.getPropertyName().equals("C_Order_ID")) {
			if (e.getNewValue() != null) {
				int C_Order_ID = ((Integer)e.getNewValue()).intValue();
				posPanel.setOrder(C_Order_ID);
		...
		try {
			...
			if (Events.ON_CTRL_KEY.equals(e.getName())) {
				...
				//Alt+I == 73
					...
					InfoPanel ip = InfoManager.create(lookupPR, null, "M_Product", "M_Product_ID", "", false, "");
					if (ip!=null)
						fldProduct.showInfoPanel(ip);
					return;
				}
				//Alt+P == 80
			...
			if (e.getTarget().equals(buttonPrint)){
				...
				InfoPanel ip = InfoManager.create(lookupBP, null, "C_BPartner", "C_BPartner_ID", "", false, "IsCustomer = 'Y' ");
				if (ip!=null)
					fldBPartner.showInfoPanel(ip);	
			}
	...
	private void deleteOrder() {
		...
		//  Ask to Open Delete Form
		FDialog.ask(posPanel.getWindowNo(), this, askMsg, new Callback<Boolean>() {

			@Override
			public void onCallback(Boolean result) 
			{
				if (result)	{
					String askMsg= posPanel.cancelOrder();
					posPanel.refreshHeader();
					posPanel.refreshPanel();
					if (askMsg !=null)
						FDialog.error(posPanel.getWindowNo(), "No se pudo borrar la orden");
				}
			}
		});

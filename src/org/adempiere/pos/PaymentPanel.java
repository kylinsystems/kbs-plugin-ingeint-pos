...
public class PaymentPanel extends Collect implements EventListener {
	...
	public Panel cashPanel(){
		...
		fPayAmt = new WNumberEditor();
		
		row.appendChild(fPayAmt.getComponent());
		fPayAmt.setValue(new BigDecimal("0.0"));
		fPayAmt.getComponent().addEventListener("onBlur",p_Event);
		fPayAmt.getComponent().addEventListener("onChange",p_Event);
		fPayAmt.getComponent().addEventListener("onFocus",p_Event);
		return mainPanel;
	}
	public Panel paymentPanel(){
		...
		AD_Column_ID = 8374; //C_Payment_v.TenderType
		MLookup cardlookup = MLookupFactory.get(Env.getCtx(), 0, 0, AD_Column_ID, DisplayType.List);
		ArrayList<Object> cards = cardlookup.getData(true, false, true, true, true);
		...
		row = rows.newRow();
		fCheckRouteNo = new WPosADTextField(p_posBasePanel, p_MPOS.getOSK_KeyLayout_ID());
		...
		fCCardNo = new WPosADTextField(p_posBasePanel, p_MPOS.getOSK_KeyLayout_ID());
		...
		fCCardName = new WPosADTextField(p_posBasePanel, p_MPOS.getOSK_KeyLayout_ID());
		...
		fCCardMonth = new WPosADTextField(p_posBasePanel, p_MPOS.getOSK_KeyLayout_ID());
		...
		fCCardVC = new WPosADTextField(p_posBasePanel, p_MPOS.getOSK_KeyLayout_ID());
...
//		processPayment(null);
	...
	public BigDecimal getPayAmt(){
		return (BigDecimal) fPayAmt.getValue();
	}
	
	
	public void showKeyboard(WPosADTextField field) {
		cont++;
		if(cont==1){
